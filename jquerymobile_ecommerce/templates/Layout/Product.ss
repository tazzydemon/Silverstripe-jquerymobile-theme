					...
					<% control Variations %>
							<tr>
								...
								<% if canPurchase %>
									<% if IsInCart %>
										<% control Item %>
											<a class="ajaxQuantityLink" data-ajax="false" href="/$removeLink" title="<% sprintf(_t("REMOVEALL","Remove one of &quot;%s&quot; from your cart"),$Title.XML) %>">
												<img src="/$ThemeDir(ecommerce)/images/minus.gif" alt="-" />
											</a>
											$QuantityField
											<a class="ajaxQuantityLink" data-ajax="false" href="/$addLink" title="<% sprintf(_t("ADDONE","Add one more of &quot;%s&quot; to your cart"),$Title.XML) %>">
												<img src="/$ThemeDir(ecommerce)/images/plus.gif" alt="+" />
											</a>
										<% end_control %>
									<% else %>
										<a href="/$Item.addLink" data-ajax="false" title="<% sprintf(_t("ADD","Add &quot;%s&quot; to your cart"),$Title.XML) %>"><% _t("ADDLINK","Add this item to cart") %></a>
									<% end_if %>
		...
		<% else %>
			...
			<% if canPurchase %>
				<% if IsInCart %>
					...
					<% end_control %>
				<% else %>
					<p class="quantityBox"><a href="/$addLink" data-ajax="false" title="<% sprintf(_t("ADD","Add &quot;%s&quot; to your cart"),$Title) %>"><% _t("ADDLINK","Add this item to cart") %></a></p>
				<% end_if %>
			<% end_if %>
		<% end_if %>
	</div>
